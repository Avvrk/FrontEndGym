...
async function editarEstado(elemento) {
    try {
        if (elemento.estado === "1") {
            await useUsuario.putUsuariosInactivar(elemento._id);
        } else if (elemento.estado === "0") {
            await useUsuario.putUsuariosActivar(elemento._id);
        }
        listarUsuarios();
    } catch (error) {
...
// Función que se encarga de validar los datos que se registrarán
    ...
    <div>
        ...
        <div id="formularioUsuario" v-if="mostrarFormularioUsuario">
            <q-form @submit="mostrarBotonEnviar ? registrar() : editar()" @reset="resetear" class="q-gutter-md">
                ...
                <q-select standout="bg-green text-white" v-model="sedeUsuario" :options="organizarSedes" option-value="valor" option-label="label" label="Sedes" color="black" />
                <div>
