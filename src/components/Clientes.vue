...
async function editarSeguimiento() {
	if (await validarDatosSeguimiento()) {
		try {
			...
			const res = await useCliente.putClientesSeguimiento(
				idSegui.value,
				info
			);
			if (res.status !== 200) {
				...
				<div id="aa" class="row">
					<q-markup-table v-for="item in segui" :key="item._id">
						...
						<button
							class="botonSeguimiento"
							@click="
								editarVistaFondoSeguimiento(true, item, false)
							">
							Editar
						</button>
					</q-markup-table>
